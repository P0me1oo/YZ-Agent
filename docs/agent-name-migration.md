...
Docker 部署更新镜像时，将配置挂载的容器内位置调整为 `/etc/yz-agent`，并核对配置中的旧绝对路径。容器外目录以实际挂载为准；systemd/OpenRC 的自动目录搬迁由安装器执行。
...
正式 `v1.14.0` 已通过 [Linux 发布 CI](https://github.com/P0me1oo/YZ-Agent/actions/runs/34874770653)，包括完整数据竞争检查及双架构程序、镜像验证。发布前补齐了错误回调下“目录尚未搬迁”和“原目录身份已改变”的两个恢复检查，并修复了不同 Bash 版本的返回状态差异。正式附件、固定来源和镜像校验值见兼容矩阵。
