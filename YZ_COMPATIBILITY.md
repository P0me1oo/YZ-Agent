...
## 连接限制与超限日志修复（v1.15.1，未发布）

- 中转修复：在共用服务层隔离落地普通用户全量、增量和轮询同步，防止内部 Shadowsocks 入站不支持用户更新后停止监听；两种核心均适用。配置重载仍更新内部凭据，失败配置不会被用户消息绕过；支持旧面板，建议配套面板 `1.18.1` 的增量推送过滤。与既有连接限制修复共用本未发布修订版本。
| 项目 | 标识 |
| --- | --- |
...
| 发布信息 | 尚未发布，Release、CI、镜像标签与 digest 待发布后补记 |
