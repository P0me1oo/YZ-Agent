...
- 当前仅本地修改，未发布。回滚基线为已发布的 `v1.17.2`；旧版仍会把内网地址计入设备数。
- 本地验证：公网地址分类、Xray 与 sing-box 两个内核的完整包测试、服务层、面板通信层和命令行版本检查均通过；`linux/amd64`、`linux/arm64` 构建通过。开发构建的 VCS revision 为修改起点 `f39d4cfa8b926dc15a5dab4bc62eaa0887e56f17`，`vcs.modified=true`，不可作为正式发布产物。面板完整 PHP 回归 294 项、3581 个断言通过，未连接真实服务器。
