...
- 本地 Windows/amd64 使用正式依赖执行 `go test ./...`，全部包通过；面板现有设备去重单测 1 项、3 个断言通过。Linux/amd64 开发构建通过，构建信息确认两个核心仍解析到固定 fork 版本、目标平台为 `linux/amd64`，当前源码提交为 `a1adc3148f30d86b2949dd0eed8a04e350f0f673`，工作区修改标记为 `vcs.modified=true`。未进行真实服务器连接测试，也未构建正式发布产物。Node `v1.17.1` 尚未发布，当前正式回滚基线仍为 `v1.17.0`。没有发布 Tag、Release 或镜像，不能用于服务器常规升级。

## Reality 传统与混合握手兼容（v1.17.1，未发布）
