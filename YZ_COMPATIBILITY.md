...
- 本地验证（2026-09-26，Windows/amd64，Go 1.27.0）：改用上述远端固定 Xray 依赖后，带完整功能标签的 Node 全部 Go 测试通过；Linux/amd64 的 `CGO_ENABLED=0` 构建通过，实际模块信息显示 Xray 固定版本和校验值。提交前本地构建的 `vcs.modified=true` 属预期，正式来源仍须由标签工作流重新构建和核对。相关 Xray 核心包通过；面板完整 PHP 回归使用 `pdo_sqlite` 通过（313 项、3,697 个断言）；管理端 `npm run verify` 通过（29 项行为测试、33 项浏览器测试和开发模式检查）。未连接真实服务器。
