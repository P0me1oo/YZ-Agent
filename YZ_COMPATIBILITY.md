...
- 来源基线为 `v1.17.4` / `022aa4c2070d806b1a87503acd4c82cd9b2b7e41`，在分支 `feat/device-limit-report` 开发。两种核心在设备名额已满、拒绝新的公网来源时，通过 limiter 记录上限、已计入来源数、被拒次数和被拒来源地址（去重后最多 5 个）；上报周期汇总后作为 `device` 类型写入已有的超限事件列表，并发和速率事件格式不变。节点指标中的设备超限累计次数改为按实际拒绝计数。
- 配套面板 `1.22.0`（未发布，分支 `feat/device-limit-report`）。旧面板丢弃 `device` 类型事件，同批其他上报照常处理，因此两边升级不分先后。核心固定依赖、节点配置和其他通信字段不变。
- 验证：本地 Windows/amd64 使用正式依赖执行 `go test ./...`，全部包通过；新增用例覆盖 limiter 的去重与截断、未知用户忽略、Xray 与 sing-box 的 TCP/UDP 拒绝记录和跨节点来源计数、上报汇总排序及事件 JSON。`go vet ./internal/...` 无问题，Linux amd64、arm64 构建通过。未发布，未进行真实服务器联调。
