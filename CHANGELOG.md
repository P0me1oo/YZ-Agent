...
- 核心固定依赖和面板通信格式不变，兼容面板 `1.18.0`；面板 `1.18.1` 另行补齐后台生成用户与试用套餐的上限同步。Node `v1.15.1` 和面板 `1.18.1` 均未发布。
...
- 扩展两个服务管理器下的名称与目录迁移、冲突、中断和失败恢复测试；具体迁移限制见 `docs/agent-name-migration.md`。
