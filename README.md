...
当前版本 `v1.19.0`：分别经 IPv4、IPv6 向面板回报公网地址，双栈服务器可同时显示两个地址；设备数超限时上报拒绝事件；并包含 `v1.18.0` 的 Xray REALITY 防盗用模式（见 [docs-reality-anti-abuse.md](docs-reality-anti-abuse.md)）。配套面板 `1.23.0`、管理端工程 `0.6.0`。
